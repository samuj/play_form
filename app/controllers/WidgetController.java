...
public class WidgetController extends Controller {
    ...
    public Result createWidget(Http.Request request) {
        ...
        if (boundForm.hasErrors()) {
            ...
            Widget widget = new Widget(data.getName(), data.getAge(), data.getBranch(), data.getMarks());
            widgets.add(widget);
            widget.save();
            return redirect(routes.WidgetController.listWidgets())
                .flashing("info", "Person added!");
        }
    }

    public Result findWidgetById( Integer id) {
      Widget widget = widgets.get(id-1);
